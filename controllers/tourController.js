...
exports.getTour = catchAsync(async (req, res, next) => {
    /* Learn:The values that come from the URL parameters are string. So in this case, we need to convert the id parameter to number for
    *   comparison in find() method. So we can multiplies it by 1. This is a nice trick that when we multiply a string that looks like
    *   a number (like '23') with a real number, that string will then automatically converted to a number.
    *
    * Also tours[tours.length - 1] is the last item in tours array.
    *
    const id = req.params.id * 1;
    const tour = tours.find(el => el.id === id);*/
    /*  * We want to check if the inputted id is valid or not? (if it exists in our file or not?)
    ...
    /* First solution:
    I commented out these lines, because we are using this exact code in param middleware in this file to avoid repeatness of code.
    ...
    */
    ...
    const tour = await Tour.findById(req.params.id);
    if (!tour) {
        /* In JS, null is a fallacy value or in other words a value that will convert to false. So if there is no tour, we want
...
exports.createTour = catchAsync(async (req, res, next) => {
    ...
    /* Without database:
     When we create a new object, we never specify the id of that object in the request.So the DB should take care of figuring
    * out the id itself.
    * Learn: Object.assign() allows us to create a new object by merging two existing objects together.

    const newId = tours[tours.length - 1].id + 1;
    const newTour = Object.assign({id: newId}, req.body);
    tours.push(newTour);

     Which one should we use? .writeFile() or .writeFileSync() ? .writeFile() because we are inside of a callback function that
    * is gonna run in the event loop. So we can never ever block the event loop and in the () of the .writeFile() we want to pass in
    * a callback function that is gonna be processed in the background and as soon as it's ready, it's gonna put it's event in one of
    * the event loop queue, which then is gonna be handled as soon as the event loop passes the phase.
    fs.writeFile(`${appFileName}/../dev-data/data/tour-simple.json`, JSON.stringify(tours), (err) => {
        if (err) {
            console.log(err);
        }
         Status 201 means : created (we created a new resource on the server). (200 means okay.)
         We don't use results property in response object here. Because we have only one result.
        res.status(201).json({
            status: 'success',
            data: {
                tour: newTour
