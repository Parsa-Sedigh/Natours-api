...
exports.signup = catchAsync(async (req, res, next) => {
  // This has serious security flow: const newUser = await User.create(req.body);
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!user || !(await user.correctPassword(password, user.password))) { // !correct means correct is falsy
...
exports.protect = catchAsync(async (req, res, next) => {
  // 1) Getting the token and check if it's there(check if it exists)
  let token;
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    // roles would be an array like: ['admin', 'lead-guide']
