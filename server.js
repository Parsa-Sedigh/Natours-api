...
/* First we have to read or configure the .env file in our project and THEN use any file that is using process.env.<name of env
...
/* eslint-config-prettier will disable formatting for eslint, so with this package, prettier will take care of formatting and not
* eslint.
* eslint-plugin-prettier package will allow eslint to show formatting errors as we type using prettier.
* So the recipe for prettier and eslint is: npm i eslint prettier eslint-config-prettier eslint-plugin-prettier eslint-config-airbnb
*  eslint-plugin-node eslint-plugin-import eslint-plugin-jsx-a11y eslint-plugin-react --save-dev
* these packages must installed locally on the project and not globally.
* airbnb style guide for javascript is very good.
* eslint-plugin-node will add couple of specific eslint rules only for nodejs. (For finding some errors when writing node.js)
* The 3 packages only neccessary in order to make the airbnb style guide actually works. So that airbnb depends on these 3 :
* eslint-plugin-import eslint-plugin-jsx-a11y eslint-plugin-react
* We need to save all of these packages as dev dependency.
* Next we need config files for both prettier and eslint.
*
* "consistent-return" in eslint says each and every function should always return something.
*/
